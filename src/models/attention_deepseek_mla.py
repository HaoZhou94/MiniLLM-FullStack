...
class DeepSeekMLA(nn.Module):
    ...
    def _create_casual_mask(self, seq_len:int, device) -> torch.Tensor:
    ...
    def forward(
        ...
        # 4. 应用因果掩码（核心：确保自回归特性）
        casual_mask = self._create_casual_mask(seq_len, device)
        ...
        # 8. 拼接多头结果
        context_layer = self._merge_heads(context_layer) # [batch_size, seq_len, all_head_size]
...
#         return x.contiguous().view(batch_size, -1, self.all_head_size)
#
#     def _create_casual_mask(self, seq_len: int, device: torch.device) -> torch.Tensor:
#         """
